...
@interface CaptureFaceService()
{
    ...
    CascadeClassifier  _smileCascade;
    BOOL               _haveTask;
    BOOL               _isCaptureing;
}
...
- (void)setupDetector{
    ...
    /** 眼睛识别 */
    NSString *eyesCascadePath = [[NSBundle mainBundle] pathForResource:@"haarcascade_eye" ofType:@"xml"];
    ...
    /** 笑脸识别 */
    NSString *smileCascadePath = [[NSBundle mainBundle] pathForResource:@"haarcascade_smile" ofType:@"xml"];
    CFStringGetFileSystemRepresentation((CFStringRef)smileCascadePath, CASCADE_NAME, CASCADE_NAME_LEN);
    _smileCascade.load(CASCADE_NAME);
...
- (std::vector<cv::Rect>)checkFacesWithImage:(cv::Mat &)image{
    ...
    double scalingFactor = 1.1; //1.1
    int minRects = 2;
    cv::Size minSize(50,50);//30
...
#pragma mark - opencvdelegate OpenCV的代理方法
